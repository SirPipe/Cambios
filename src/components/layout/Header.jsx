...
const Header = () => {
  ...
  const handleTopLinkClick = (targetPath) => (event) => {
    ...
    <header className={`header ${scrolled ? 'header-scrolled' : ''}`}>
      <div className="header-container">
        <div className="logo-container" style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
          ...
          >
            <svg viewBox="0 0 868.7 760.7" xmlns="http://www.w3.org/2000/svg" className="header-brand-icon" style={{ width: '90px', height: '90px' }}>
            ...
            {/* <span className="header-brand-text">CAMBIANDO LA COSTA</span> */}
          </Link>
        </div>
        <nav className="nav-links">
          <Link to="/quienes-somos" onClick={handleTopLinkClick('/quienes-somos')}>¿Quiénes somos?</Link>
          <a href={location.pathname === '/quienes-somos' ? '#contactanos-form' : '/quienes-somos#contactanos-form'}>
            Contáctanos
          </a>
        </nav>
      </div>
